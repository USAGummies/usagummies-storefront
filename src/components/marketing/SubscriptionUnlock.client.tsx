...
export function SubscriptionUnlock({
  ...
  if (!unlocked) {
    return (
      ...
      >
        <div className={cn("text-sm font-black", variant === "dark" ? "text-white" : "text-[var(--text)]")}>
          Subscription access
        </div>
        <div className={cn("mt-1 text-xs", variant === "dark" ? "text-white/65" : "text-[var(--muted)]")}>
          Subscriptions unlock after your first order. Premium access only.
        </div>
        <div className={cn("mt-3 inline-flex items-center rounded-full border px-3 py-1 text-[11px] font-semibold",
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "mt-3 inline-flex items-center rounded-full border px-3 py-1 text-[11px] font-semibold",
          variant === "dark" ? "border-white/20 text-white/70" : "border-[var(--border)] text-[var(--muted)]"
        )}
      >
        Eligible now
