...
// `@/lib/cart` uses `next/headers` (server-only). From a client component
// we must call the server-action wrapper instead, which handles cookies
// on the server.
...
function ReorderControls({ intent }: { intent: ReorderIntent }) {
  ...
  async function handleClick() {
    ...
    setError(null);
    try {
      // Sequential calls through the single-variant `addToCart` server
      // action (FormData-based). Shopify computes the current price.
      for (const item of intent.addable) {
        const fd = new FormData();
        fd.set("merchandiseId", item.variantId);
        fd.set("quantity", String(item.quantity));
        const res = await addToCart(fd);
        if (res && res.ok === false) {
          throw new Error(res.error || "Cart add failed.");
        }
      }
      setState("added");
    } catch (err) {
