...
• AMAZON FBA ACCOUNTING: Amazon is a consignment model, NOT a wholesale PO/invoice model. Shipping TO Amazon = inventory transfer (still our asset, no revenue). Revenue is recorded when Amazon SELLS units. Amazon fees (referral ~15%, FBA fulfillment, storage, PPC) are Selling Expenses, not COGS.
• AMAZON RECORDING CADENCE RULE: Use bi-weekly settlement-based recording (Option A) until Amazon volume exceeds 1,000 units/month, then switch to daily recording (Option B). CRITICAL EXCEPTION: During the final 2 weeks of each fiscal QUARTER (3/31, 6/30, 9/30) and YEAR-END (12/31), ALWAYS switch to daily recording regardless of volume. This ensures quarterly financial statements have accurate revenue, COGS, and receivables, and the fiscal year closes with precise figures. After the period close, revert to settlement-based recording.
• NOTE: The $1.52/unit cost used in examples is a PLACEHOLDER. Actual unit cost will be calculated from the final Powers + Belmark invoices divided by actual units received.
