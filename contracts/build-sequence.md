...
- Sample Order Dispatch (S-08) runtime — event-driven Class-B proposal agent per `/contracts/agents/sample-order-dispatch.md`. 🟢 **MVP + Shopify + HubSpot + Amazon FBM adapters LIVE** (2026-04-21). Classifier + composer + dispatch route + Shopify `orders/paid` webhook + HubSpot `deal.propertyChange` webhook + Amazon FBM alert cron + Amazon dispatch bridge. 45/45 vitest green across three adapters. Faire adapter remains blocked on `FAIRE_ACCESS_TOKEN`.
- `mailer` packaging type — 9×6×2 in, 0.55 lb. Single-bag DTC + Amazon FBM. 🟢 **LIVE** (2026-04-21). Threaded through shipstation-client PACKAGE_PROFILES + atp-gate BAGS_PER_CARTON + buy-label + preflight + freight-shop + S-08 classifier.
- Inventory cover-day forecast (Inventory Specialist S-07 MVP) — 🟢 **LIVE** (2026-04-21). `/api/ops/inventory/cover-days` reads snapshot + env burn rate, returns per-SKU + fleet cover-days, urgency buckets (≤14d urgent / ≤30d soon), reorder recommendations. Env `INVENTORY_BURN_RATE_BAGS_PER_DAY` (default 250) + per-SKU overrides.
- Compliance Specialist `[FALLBACK]` doctrine list — 🟢 **LIVE** (2026-04-21). When Notion `/Legal/Compliance Calendar` is missing, the agent posts the 11-category doctrine list (WY annual report, federal 1120, USPTO §8/§9, FDA FFR, insurance, WA business license, vendor COIs) with `[FALLBACK]` tags + real date sources. No fabricated dates. Once the Notion DB lands, live-mode takes over automatically.
- `/ops/ledger` Rene finance dashboard — 🟢 **LIVE** (2026-04-21). One-page: CF-09 freight-comp queue with inline Approve+Post / Reject + stale void review.
