...
- **0.3 — 2026-04-30** — SMS to buyer with prefilled NCS-001 deeplink (`src/lib/sales-tour/sms-buyer.ts`) + real-time HubSpot deal autosync from booth quote (`src/lib/sales-tour/hubspot-autosync.ts`). Booth route gains `noBuyerSms` + `noHubSpotAutosync` body fields. Buyer SMS is opt-in (requires `SALES_TOUR_BUYER_SMS_ENABLED=true` AND captured buyer phone) so a misconfigured deploy can't blast strangers. HubSpot autosync creates contact (upsert by email when captured) + deal at the right stage (`STAGE_QUOTE_PO_SENT` for on-grid quotes, `STAGE_LEAD` for non-grid quotes that need deal-check). Public-copy rules honored — buyer SMS body never names Ben's full name or the warehouse city. NCS deeplink: `https://www.usagummies.com/upload/ncs?co=<company>&ref=<tourId-visitId>`. Required env (additive): `SALES_TOUR_BUYER_SMS_ENABLED=true`, `HUBSPOT_PRIVATE_APP_TOKEN` (existing). v0.3 graduation criteria: trip ends with all booth visits in HubSpot + audit trail clean.
