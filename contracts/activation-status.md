...
| CRM Hygiene (Wholesale Pipeline Commander) | [`src/lib/sales/crm-hygiene/`](../src/lib/sales/crm-hygiene/) | [`/api/ops/agents/crm-hygiene/run`](../src/app/api/ops/agents/crm-hygiene/run/route.ts) | Weekday 14:15 UTC (07:15 PT) | `#sales` — daily HubSpot scan: missing-field, stale-deal, zero-dollar, stuck-in-stage, duplicate-name, closed-with-open-amount. Quiet-collapse on zero findings. |
| DTC Revenue Commander | [`src/lib/sales/dtc-revenue/`](../src/lib/sales/dtc-revenue/) | [`/api/ops/agents/dtc-revenue/run`](../src/app/api/ops/agents/dtc-revenue/run/route.ts) | Weekday 15:30 UTC (08:30 PT) | `#ops-daily` — daily Shopify pulse with day-over-day, week-over-week, 7-day rolling, and MTD context. Reads `kpi_timeseries`. GA4 conversion data not yet wired server-side. |
...
| Agent Status UI | — | [`/ops/agents/status`](../src/app/ops/agents/status/page.tsx) + [`/api/ops/agents/status`](../src/app/api/ops/agents/status/route.ts) | Polled every 60s (UI) | Per-agent green/yellow/red health cards, 13 agents tracked |
