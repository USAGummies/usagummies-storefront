...
| **B3** | Master carton (36 bags) | $3.50 / bag | Buyer pays freight | Online wholesale dashboard |
| **B4** | Pallet | $3.25 / bag | LANDED — USA Gummies ships, freight built in | Online wholesale dashboard |
| **B5** | Pallet | $3.25 / bag | Buyer pays freight | Online wholesale dashboard |
...
- **2.5 — 2026-05-03 (latest)** — *Both-paths-stay-open clarification + B6-ANCH proposal shelved.* Per Ben (Slack DM 2026-05-03 PM in response to Claude's #financials Q3/Q4 follow-up): keep BOTH the landed and buyer-pays freight tiers fully quote-eligible — *"we need to leave both paths open so we can quote with and without freight."* The v2.4 +$0.25 surcharge ratification stands as captured (B3 $3.50, B5 $3.25); the price-gap-nudges-buyers-toward-landed framing was a rationalization, NOT a deprecation of buyer-pays. **Hard rule:** quote buyer-pays as a real first-class option whenever the buyer signals freight-account preference or large-format pickup intent. The B-grid does NOT collapse into landed-only. The B6-ANCH route-anchor pallet proposal ($3.00/bag landed @ 3-pallet min) is **shelved** until a real route-anchor customer requests it — no sixth designator before there's a corridor to anchor. When that customer surfaces, default Class C `pricing.change` (Ben + Rene) per `/contracts/approval-taxonomy.md` § Class C; no separate "first-time vs reorder" class fork at this scale. No code change in this version (doctrine clarification only).
- **2.4 — 2026-04-30 PM** — *Q3 buyer-pays surcharge ratified* (Class C `pricing.change`, Ben approved 2026-04-30 PM "ok"). Per Rene's 4/30 AM directive: across-the-board +$0.25/bag surcharge on buyer-pays freight tiers because *we still have to handle and should get something for it* + the price-gap nudges buyers toward landed (we ship easier, more revenue, less coordination). Two B-tier price changes:
  - `B3` master carton buyer-pays: $3.25 → **$3.50/bag** (now $0.01 above B2 $3.49 landed → landed becomes the cheaper option, buyers self-select into it)
  - `B5` pallet buyer-pays: $3.00 → **$3.25/bag** (matches B4 $3.25 landed at parity → landed is easier for the buyer with no price penalty)
  - `B2` $3.49 landed and `B4` $3.25 landed unchanged
  - Distributor delivered prices ($2.10 Inderbitzin/Glacier · $2.49 sell-sheet) unchanged — they're already on landed agreements per `/contracts/distributor-pricing-commitments.md`
  Source: Slack `#financials` thread + Ben + Rene DM 2026-04-30 AM/PM. Code-side mirror: `src/lib/wholesale/pricing-tiers.ts` price-table needs B3 + B5 number bumps + the booth-quote engine + invoice templates need to reflect the new numbers (queued as a Phase 35.g code commit; this contract update is the doctrine lock that Phase 35.g implements). The per-channel proforma sheet `/contracts/proforma-channel-margins.md` also needs B3 + B5 GP/bag updates. No invoice reissues required for live customers — first quote at the new B3/B5 numbers is still in front of us (no live B3 or B5 customer at the moment of the change).
