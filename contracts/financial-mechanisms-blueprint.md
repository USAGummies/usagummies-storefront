...
Per `/contracts/email-agents-system.md` §6. **Status: 🟢 37.1 + 37.2 + 37.3 + 37.4 + 37.5 + 37.6.a + 37.7 SHIPPED · 37.6.b/c + 37.8–37.16 queued.** Ships INSIDE Viktor as workflow lanes (no new runtime agent).

37.1 Inbox Scanner ✅ · 37.2 Classifier (deterministic layer) ✅ · 37.3 HubSpot Verification cross-cut (pre-classify enrichment) ✅ · 37.4 Validator extension ✅ · 37.5 Strategic Framework analyzer ✅ · 37.6.a Slack approval-card builder ✅ (37.6.b interactivity webhook + 37.6.c LLM-edit modal queued) · 37.7 Spam Cleaner (Class A-d) ✅ · 37.8 Bounce/Routing/OOO · 37.9 Sample Shipper (S-08 wrap) · 37.10 Whale Escalator · 37.11 Polite-No/Qualify/Thread-Fix · 37.12 Receipt+AP Handler · 37.13 Memory layer · 37.14 Daily-brief surface · 37.15 Weekly Audit (Drift Audit Runner extension) · 37.16 Tests + drift-audit fixtures
...
**37.3 ship log (2026-05-01 AM):** `src/lib/sales/viktor/hubspot-verification.ts` — pre-classify enrichment lane of §2.9. Class A `system.read` lookups via existing `hubspot-client.findContactByEmail` + `getContactById`. Per-record decision: whale-domain HARD BLOCK (short-circuits before any HubSpot call) → UNQUALIFIED HARD BLOCK → §11.6 gate check (usa_vertical/usa_tier/usa_cadence_state populated?) → soft missing-contact. Skipped categories (Z/N/O/P/_unclassified) bypass HubSpot entirely to preserve rate-limit budget. `maxLookups` cap (default 30/run) prevents misconfigured-window blowups. HubSpot-not-configured wholesale-degrades cleanly. Persists `hubspot` metadata onto each inbox:scan record. 23-test coverage. Chained third in the cron pipeline — scanner → classifier → verification all run as one heartbeat. **Pre-send check (§2.9 #1) and daily reconciliation (§2.9 #3) deferred** to Phase 37.5/37.6 (drafter) and Phase 37.15 (weekly audit) respectively, as those are owned by separate doctrine sections.

**37.4 ship log (2026-05-01 AM):** `src/lib/sales/viktor/validator.ts` — pure-function outbound-draft gate per §2.4. Eight finding classes: `compliance` (kosher/halal/gelatin-source/organic/non-GMO/vegan/dairy-free/peanut-free), `pricing` (per-bag $ outside canonical grid {$3.25, $3.49, $3.00, $4.99, $5.99} — distributor figures $2.10/$2.49 are internal-only and HARD BLOCKED in customer copy), `internal-info` (Ashford/route-doctrine/B-tier-code/COGS leakage), `hold-pattern` (whale-domain mention, exclusivity, multi-year, MNDA, indemnification — Class D HARD HOLD), `regulatory-tailwind` (TX SB 25 / CA AB 418 / CA AB 2316 / FDA dye-ban / warning-label per §3.6 internal-only doctrine), `fabricated-fact` (Layton/wrong-flavor-count/"apple"-not-"green-apple"/resealable/red-white-blue/wrong-pallet-math), `tag-hold` (explicit drafter marker), `missing-anchor` (cold-outreach soft warning). 53-test coverage. Battle-tested regexes from `scripts/outreach-validate.mjs` ported verbatim — the .mjs CLI script remains as a manual pre-send check; this module is what the runtime drafter (37.5/37.6/37.11) calls on every output. Includes `renderValidationSummary()` for §2.5a Slack approval-card surface. **Apollo / HubSpot / Gmail dedup network checks remain in the .mjs script** — those belong in 37.5/37.6 (drafter), not the pure-function validator.

**37.6.a ship log (2026-05-01 AM):** `src/lib/sales/viktor/approval-card.ts` — pure Block Kit JSON builder for the §2.5a approval-card surface. `buildApprovalCard({draftId, recipient, subject, draftBody, frame, validation, category, classLevel})` returns `{blocks, text, actionIds}` ready to drop into `postMessage({channel, blocks, text})`. Top-down section order per spec: header → recipient/subject context → optional validator block (only when blockers OR warnings present) → strategic-frame block (uses `renderStrategicFrameForCard()` from 37.5) → draft-body block (mrkdwn code-block, truncates at 2900 chars per Slack section limit) → actions row with 3 buttons. Approve button: style=primary on clean validator + Class B; no style + "(blocked)" label when validator reports hard-blockers; Class C/D add `confirm` dialog (extra-confirm step for high-stakes sends). Action ids per spec: `email_approve_<draftId>`, `email_deny_<draftId>`, `email_edit_<draftId>`. `parseEmailActionId()` reverse-parser for the webhook handler. `approvalCardChannel(class)` routes B → `#ops-approvals`, C/D → `#financials` (env-overridable). 20-test coverage (section-order with/without validator block, action_id format + constants exposure, parseEmailActionId round-trips + non-email rejection, Approve button styling per class + blocked state, Deny always danger, channel routing, draft body verbatim + truncation). **37.6.b (interactivity webhook handler) and 37.6.c (LLM-edit modal) deferred** to separate atomic ships — those are stateful surfaces touching network + LLM. Total Viktor namespace now at 231 green tests across 7 modules.

**37.5 ship log (2026-05-01 AM):** `src/lib/sales/viktor/strategic-frame.ts` — the canonical `StrategicFrame` typed struct per §2.5b + 8-question deterministic builder. Pure-function `buildStrategicFrame({record, verification?, prospect?, cashflow?})` derives premise/relationship/opportunity/goal/risks/financialFrame/dontShare/play from classifier + HubSpot enrichment + ProspectFrame (vertical/tier) + CashflowFrame (cashSpeed). Whale-class records always elevate to relationship=whale + goal=hold + Class C requirement (defense-in-depth). Tier-based opportunity scaling: T0 ×50, T1 ×10, T2 ×3, T3 ×1. Cashflow elevation: cashSpeed=today/this_week + opportunity ≥ $10K → requiresClassC=true. Universal `dontShare` floor of 9 entries (route doctrine, COGS breakdown, custom-quote formula, per-vendor margin, internal warehouse address, B-tier codes, regulatory tailwind, supplier names, governance vocabulary) on every frame. Per-category playbooks for all 22 v1 categories — empty-risk categories filled out so validator passes. Hard-rule pre-draft gate: `validateStrategicFrame(frame)` rejects null / missing premise / empty risks / missing financial / empty dontShare / empty play — drafter MUST call this before generating reply per §2.5b. `renderStrategicFrameForCard(frame)` formats for §2.5a Slack approval card top-of-message. 48-test coverage (sample/pricing/polite-no/whale defaults · tier scaling · cashflow elevation · dontShare composition · validation rejection paths · render coverage · per-category yields-valid sanity check). Total Viktor namespace: 211 green tests across 6 modules.

**37.7 ship log (2026-05-01 AM):** `src/lib/sales/viktor/spam-cleaner.ts` — the FIRST Class A-d (autonomous DELETE) lane in the system per §2.5d + §2.8. Detection rules (ALL must be true): denylist sender + noise subject + no HubSpot engagement + no attachment. Five HARD STOPS: whale-domain (defense-in-depth), HubSpot prior engagement (§7.13 permanent), subject safety pattern (invoice/W-9/PO/sample request/order confirmation/quote — never trash), attachment, denylist + safety-pattern miss. Auto-delete is GATED behind `SPAM_CLEANER_AUTO_DELETE=true` env flag — default = dry-run. Detection runs unconditionally and surfaces candidates in the daily digest so the operator can audit volume + per-domain mix BEFORE flipping the flag. Posts daily digest to `#ops-audit`. Persists daily KV log at `spam-cleaner:log:<YYYY-MM-DD>` (90-day TTL for audit retention). New `moveToTrash()` primitive in `gmail-reader.ts` (uses new `gmail.modify` scope client). Cron `0 8 * * *` UTC = 1 AM PT off-hours daily. 37-test coverage (matchSubjectPattern + matchSafetyPattern parameterized tables, 4 HARD-STOP paths, eligibility, dry-run defaults, env-flag flip, real trash success/failure, byDomain counts, KV log persistence, digest renderer empty/non-empty/dry-run/failure variants). Total Viktor namespace: 163 green tests across 5 modules.
