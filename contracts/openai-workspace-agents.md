...
- `ops.inbox.unified` is a ready read-only registry entry backed by `/api/ops/inbox` and `/ops/inbox`, giving ChatGPT workspace agents inbox context without enabling AI triage, drafting, sending, archiving, deleting, labels, or the email-intel runner.
- `ops.email-agents.status` is a ready read-only registry entry backed by `/api/ops/email-agents/status` and `/ops/email-agents`, giving ChatGPT workspace agents the email-agent readiness gates without triggering Gmail, Slack approvals, HubSpot writes, or the email-intel runner.
- `ops.email-agents.readiness-dry-run` is a ready read-only registry entry backed by `/api/ops/agents/email-intel/run`. It returns a canonical heartbeat run record from the same readiness gates and appends only a fail-soft internal audit row; it never invokes `/api/ops/fulfillment/email-intel/run`, Gmail drafts, Slack approvals, or HubSpot writes.
- `ops.email-intel.run.direct` is explicitly prohibited/blocked. The email-intel auto-replier remains disabled after the 2026-04-30 incident; ChatGPT cannot trigger `/api/ops/fulfillment/email-intel/run`.
