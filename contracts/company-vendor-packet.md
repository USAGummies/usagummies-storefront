...
| Ingredients (canonical) | **Corn syrup, sugar, water, gelatin, citric acid, natural flavor, pectin, colors (from fruits, vegetables, spirulina, and curcumin), vegetable oil (coconut, canola), carnauba leaf wax (to prevent sticking)** | Source: `src/app/ingredients/page.tsx` `INGREDIENTS_LIST` — matches the printed bag panel published on usagummies.com/ingredients. 10 ingredients total. |
| Gelling agents | **Gelatin (primary) + pectin (for texture stability)** | Both appear in the canonical list above. When asked "pectin or gelatin?" the dominant-truth answer is gelatin; the precise answer is "both, gelatin is primary." |
| Allergens | *"Contains gelatin. No major Big-9 allergens (peanut, tree nut, dairy, egg, wheat, soy, fish, shellfish, sesame)."* | Big-9 absence is verified by ingredient list above. No tree nut / peanut / dairy / egg / wheat / soy / fish / shellfish / sesame ingredient appears. |
| Gelatin source | 🚩 **UNSPECIFIED** in current company doctrine | `/contracts/made-in-usa-substantiation.md` lists three candidate domestic suppliers (Gelita Sergeant Bluff IA / Rousselot Dubuque IA / PB Gelatins Little Rock AR) with a "blended-lot risk" caveat. The actual lot's gelatin source has NOT been canonicalized — Drew owns the Albanese vendor relationship and can pull the spec sheet for our specific raw-material lot. **Until specified: do not state "beef-derived" or "pork-derived" — only state "Contains gelatin."** |
...
- **v1.3 — 2026-04-30 PM (latest)** — Located canonical ingredient list in our own repo: `src/app/ingredients/page.tsx` `INGREDIENTS_LIST` (the array that renders on `usagummies.com/ingredients`, mirroring the printed bag panel). Updated §5 with the full 10-ingredient declaration. Confirmed gummy is gelatin-primary + pectin-supplementary (Vahag's "pectin or gelatin?" answer is technically "both, gelatin primary"). Gelatin source remains UNSPECIFIED — `/contracts/made-in-usa-substantiation.md` lists three candidate suppliers but the actual lot's source has not been canonicalized; Drew owns the Albanese relationship and pulls the spec sheet on demand.
- **v1.2 — 2026-04-30 PM** — Walked back unsourced "gelatin (beef-derived)" claim in §5. The original v1.0 wording asserted a specific source without primary-source verification.
